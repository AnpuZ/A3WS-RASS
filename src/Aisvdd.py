...
class Aisvdd:
    def __init__(self, data, sigma, score_bound, alpha_bound, eps_ol, eps_cp=1e-8):
        ...
        alpha_bound: the alpha bound for selecting support vectors 
        score_bound: the score bound for selecting support vectors
    ...
    def find_sv(self):
        ...
        """
        AISVDD main function.
        """
        for new_data in self.data[1:]:
            new_data = np.array([new_data])

            score, sim_vec = self.score_fcn(new_data)
            if score > self.score_bound:                
                temp_inv_A = self.expand(new_data, sim_vec)
                if temp_inv_A is None:
                    continue

                if min(self.alpha) < self.alpha_bound:                    
                    backup = self.shrink()
                    for each in backup:
                        each = np.array([each])
    ...
    def up_inv(self, prev_inv, v):
        ...
        """
        Calculate the inverse of A_(k+1) based on Lemma 3.1.
        Inputs
        ----------
    ...
    def down_inv(self, next_inv):
        ...
        """Calculate the inverse of A_k based on Lemma 3.2.
        Inputs
        ----------
    ...
    def expand(self, new_sv, new_sim_vec):
        ...
        """Expand the support vector set according to algorithm 2.
        Inputs
        ----------
        new_sv: The new support vector
        """

        temp_inv_A = self.up_inv(self.inv_A, new_sim_vec)
        if temp_inv_A is None:
            return
        else: 
            self.inv_A = temp_inv_A
    ...
    def shrink(self):
        ...
        """
        Shrink the support vector set according to algorithm 3.
        """
